...
void main() {
  ...
  group('R2: Gestión de lugares de interés', () {
    ...
    test('H5-E1V - Crear lugar', () async {
      ...
      expect((firstLocationh5e1.getCoordinate().getLat - lath5e1).abs() < 0.001,
          equals(true)); // Verifica la latitud
      expect(
          (firstLocationh5e1.getCoordinate().getLong - longh5e1).abs() < 0.001,
    ...
    test('H5-E3I - Coordenadas del Lugar incorrectas', () async {
      ...
      when(mockDbAdapterLocation.createLocationFromCoord(any))
          .thenThrow(Exception("Coordenadas inválidas"));
    ...
    test('H6-E1V - Crear lugar', () async {
      ...
      // Verificar que los valores del primer lugar son los esperados
      expect((firstLocationh5e1.getCoordinate().getLat - lath6e1).abs() < 0.001,
          equals(true)); // Verifica la latitud
      expect(
          (firstLocationh5e1.getCoordinate().getLong - longh6e1).abs() < 0.001,
          equals(true)); // Verifica la longitud
      expect(firstLocationh5e1.getToponym(),
    ...
    test('H6-E3I - Toponimo del Lugar incorrecto', () async {
      ...
      // GIVEN
      String emailh6e3 =
          "Pruebah6e3${DateTime.now().millisecondsSinceEpoch}@gmail.com";
      ...
      // Simular la creación de un lugar que arroja una excepción
      when(mockDbAdapterLocation.createLocationFromTopo(any))
          .thenThrow(Exception("Toponimo inválido"));
    ...
    test('H7-E1V - Listar lugares', () async {
      ...
      when(mockDbAdapterLocation.getLocationList()).thenAnswer(
        (_) async => {
          Location(Coordinate(lat1, long1), alias1h7e1, topoh7e1),
        },
      );
      ...
      expect((firstLocation.getCoordinate().getLat - lat1).abs() < 0.001,
          equals(true)); // Verifica la latitud
      expect((firstLocation.getCoordinate().getLong - long1).abs() < 0.001,
          equals(true)); // Verifica la longitud
      expect(
          firstLocation.getToponym(), equals(topoh7e1)); // Verifica el toponimo
    ...
    test('H8-E1V - Eliminar lugar de interés', () async {
      ...
      final locationMock =
          Location(Coordinate(lath8e1, longh8e1), topoh8e1, aliash8e1);

      when(mockDbAdapterLocation.createLocationFromCoord(any))
          .thenAnswer((_) async => true);

      await locationController.createLocationFromCoord(
          lath8e1, longh8e1, aliash8e1);

      // THEN
      when(mockDbAdapterLocation.deleteLocation(any))
          .thenAnswer((_) async => true);
      when(mockDbAdapterLocation.getLocationList()).thenAnswer((_) async => {});

      await locationController.deleteLocation(locationMock);
      final Set<Location> locationList =
          await locationController.getLocationList();
