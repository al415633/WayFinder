...
class Petrolcarprice implements Price {
  @override
  Future<double> calculatePrice(Routes route, Vehicle vehiculo) async {
    double distance = route.distance;
    double consumption = vehiculo.consumption;

    double pricePerlitre = await GasoilGasolina.fetchPrice(route, vehiculo.fuelType);
    if (pricePerlitre == -1) {
      throw Exception('Error en el cálculo del precio del combustible');
    }
